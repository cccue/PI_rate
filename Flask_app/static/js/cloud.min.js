function generate() {
    ...
    var spiral_type = "archimedean", font_type = "Impact", scale_type = "log", max_words = 40;

    layout.font(font_type).spiral(spiral_type),
    fontSize = d3.scale[scale_type]().range([10, 100]),
    tags.length && fontSize.domain([+tags[tags.length - 1].value || 1, +tags[0].value]),
    complete = 0,
    statusText.style("display", null ),
    words = [],
    layout.stop().words(tags.slice(0, max = Math.min(tags.length, +max_words))).start()
}
...
var fill = d3.scale.category20b(), w = 600, h = 600, words = [], max, scale = 1, complete = 0, keyword = "", tags, fontSize, maxLength = 30, fetcher, statusText = d3.select("#status"), layout = d3.layout.cloud().timeInterval(10).size([w, h]).fontSize(function(t) {
    return fontSize(+t.value)
}).text(function(t) {
    return t.key
}).on("word", progress).on("end", draw);

$(".cloud-button").click(function(e) {

    var line="./make_cloud/" + document.getElementById('rum_index').value;
    $.getJSON(line, function(data) {
      tags = data;
      if ( $("#svd_vis").length ) {
         generate();
      } else {
      svg = d3.select("#vis").append("svg").attr("width", w).attr("id","svd_vis").attr("height", h), background = svg.append("g"), vis = svg.append("g").attr("transform", "translate(" + [w >> 1, h >> 1] + ")")
        generate(); 
      }
    });  
